...
class CacheConsultas extends Interceptor {
  ...
  bool _permitido(RequestOptions opcoes) {
    if (escopo.isEmpty || opcoes.method != 'GET' ||
        opcoes.extra['semCache'] == true || opcoes.baseUrl != servidor) {
      return false;
    }
    ...
    return RegExp(r'^(produtos|categorias|mesas|comandas|cardapio|'
        r'itens_recorrentes|config|config_bigchef)/listar[^/]*\.php$').hasMatch(rota);
  }
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) async {
    ...
    try {
      ...
      Object? dados = consulta == null ? null : jsonDecode(consulta['valor'] as String);
      dados ??= await _derivar(options, alvo);
      if (dados != null) {
        final recente = consulta != null && DateTime.now().millisecondsSinceEpoch -
            (consulta['atualizado'] as int) < 10000;
        if (!recente) _atualizar(options, alvo);
        return handler.resolve(Response(requestOptions: options,
            statusCode: 200, data: dados, extra: {'cacheLocal': true}));
      }
  ...
  void onResponse(Response response, ResponseInterceptorHandler handler) async {
    ...
    if (alvo != null && alvo == escopo && response.extra['cacheLocal'] != true &&
        response.statusCode == 200 &&
        (response.data is List || response.data is Map) &&
        !(response.data is Map && response.data['sucesso'] == false)) {
      try {
        await banco.guardarConsulta(alvo, chave(response.requestOptions), response.data);
        confirmarConexao();
      } catch (_) {
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) async {
    if (falhaDeConexao(err)) {
      ...
      if (alvo != null && alvo == escopo) {
        try {
          final consulta = await banco.consulta(alvo, chave(err.requestOptions));
          ...
          if (dados != null) {
            return handler.resolve(Response(requestOptions: err.requestOptions,
                data: dados, statusCode: 200, extra: {'cacheLocal': true}));
          }
        } catch (_) {}
      }
    }
    ...
    unawaited(() async {
      try {
        await cliente.fetch(opcoes.copyWith(
          ...
          extra: {...opcoes.extra, 'semCache': true, 'servidorFixo': opcoes.baseUrl},
        ));
  ...
  Future<Object?> _derivar(RequestOptions opcoes, String alvo) async {
    ...
    if (rota.startsWith('produtos/')) {
      ...
      if (rota != 'produtos/listar.php' && rota != 'produtos/listar_por_categoria.php') {
        return null;
      }
      ...
      if (categoria != '0' && categoria.isNotEmpty) {
        final idsPromocao = (catalogo['categoriasEspeciais'] as Map?)?[categoria] as List?;
        itens = itens.where((p) => idsPromocao != null
            ? idsPromocao.contains(p['id']) : p['categoria'] == categoria).toList();
      }
      if (rota == 'produtos/listar.php') {
        final pesquisa = (q['pesquisa'] ?? '').trim().toLowerCase();
        final exato = q['codigo_exato'] == 'Sim';
        itens = itens.where((p) => exato
            ? _codigo(p['codigo'].toString()) == _codigo(pesquisa)
            : ['nome', 'codigo', 'valorVenda'].any((campo) =>
                p[campo].toString().toLowerCase().contains(pesquisa))).toList();
        itens.sort((a, b) => a['nome'].toString().compareTo(b['nome'].toString()));
        return itens;
      }
    ...
    if (q['pesquisa']?.isNotEmpty == true) {
      final semBusca = opcoes.copyWith(path: rota,
          queryParameters: {...q, 'pesquisa': ''});
      ...
      bool corresponde(Map item) => ['nome', 'codigo', 'nomeCliente', 'obs']
          .any((key) => (item[key] ?? '').toString().toLowerCase().contains(termo));
      if (dados is List) {
        final campo = rota.startsWith('mesas/') ? 'mesas' : 'comandas';
        return dados.whereType<Map>().map((e) => e[campo] is List
            ? {...e, campo: (e[campo] as List).whereType<Map>().where(corresponde).toList()}
            : e).where((e) => e[campo] is List || corresponde(e)).toList();
      }
    }
    return null;
  }
