...
class AtendimentosLocais {
  ...
  static bool _encerradaNoServidor(Map<String, Object?> op, Map estado) {
    final real = recibo(op)['id_comanda_pedido']?.toString();
    if (real == null ||
        (estado['atendimentos'] as Map? ?? {}).containsKey(real)) {
      return false;
    }
    final original = dados(op);
    final tipo = original['tipo'];
    final chave = '$tipo:${original['id_$tipo']}';
    final atual = (estado['recursos'] as Map?)?[chave] as Map?;
    // So uma consulta posterior pode encerrar uma abertura ja confirmada.
    return atual?['versao'] != null &&
        atual!['versao'] != (original['recursos'] as Map?)?[chave];
  }
  ...
  Future<String> idServidor(String id) async {
    ...
    if (real == null || ['conflito', 'arquivado'].contains(op!['estado'])) {
      throw StateError(
          'Aguarde a confirmacao da abertura para alterar este atendimento.');
    }
  ...
  Future<String> abrir(Map<String, dynamic> dados, String destino) async {
    ...
    await banco.db.transaction((tx) async {
      final pendentes = await tx.query('operacoes',
          where:
              "escopo = ? AND acao = 'abertura' AND estado NOT IN ('arquivado', 'conflito')",
          whereArgs: [escopo]);
      final estado = jsonDecode(
          await BancoLocal.lerDocumento(tx, 'estado:$escopo') ?? '{}') as Map;
      for (final op in pendentes) {
        if (_encerradaNoServidor(op, estado)) continue;
  ...
  Future<Map<String, dynamic>> detalhe(String id) async {
    ...
    for (final pedido in await banco.db.query('operacoes',
        where:
            "escopo = ? AND atendimento = ? AND acao = 'produtos' AND estado <> 'arquivado'",
        whereArgs: [escopo, id])) {
    ...
    return {
      ...
      'id': id,
      'produtos': itens,
      'status': ['conflito', 'arquivado'].contains(op['estado']) ||
              _encerradaNoServidor(op, estado)
          ? 'Fechamento'
          : 'Andamento',
  ...
  Future<List<dynamic>> projetarLista(
      List<dynamic> grupos, String tipo, String pesquisa) async {
    final campo = tipo == 'mesa' ? 'mesas' : 'comandas';
    final ocupado = tipo == 'mesa' ? 'mesaOcupada' : 'comandaOcupada';
    final copia =
        List<Map<String, dynamic>>.from(jsonDecode(jsonEncode(grupos)) as List);
    final aberturas = await banco.db.query('operacoes',
        where:
            "escopo = ? AND acao = 'abertura' AND estado NOT IN ('conflito', 'arquivado')",
        whereArgs: [escopo]);
    final estado = jsonDecode(await banco.ler('estado:$escopo') ?? '{}') as Map;
    for (final op in aberturas) {
      ...
      if (real != null) {
        ...
        for (final grupo in copia) {
          for (final recurso in grupo[campo] as List? ?? []) {
            if (recurso['idComandaPedido']?.toString() == real.toString()) {
              recurso['idComandaPedido'] = localId;
              encontrado = true;
            }
          }
        }
        if (encontrado || _encerradaNoServidor(op, estado)) continue;
      }
      final detalhe = await this.detalhe(localId);
      if (pesquisa.isNotEmpty &&
          !['nome', 'nomeCliente', 'codigo', 'observacaoDoPedido'].any((c) =>
              (detalhe[c] ?? '')
                  .toString()
                  .toLowerCase()
      ...
      if (copia.any((g) => (g[campo] as List? ?? [])
          .any((r) => r['id'] == id && r[ocupado] == true))) {
        continue;
      }
      for (final grupo in copia) {
      ...
      (grupo[campo] as List).add({
        ...
        'ativo': 'Sim',
        ocupado: true,
        'idComandaPedido': localId,
        'idCliente': detalhe['idCliente'],
        'nomeCliente': detalhe['nomeCliente'],
        'obs': detalhe['observacaoDoPedido'],
        'nomeMesa': detalhe['nomeMesa'],
        'idmesa': detalhe['idMesa'],
        'dataAbertura': detalhe['dataAbertura'],
        'horaAbertura': '',
        'dataultimopedido': '',
  ...
  Future<String> nomeCliente(String id) async {
    final chave = CacheConsultas.chave(RequestOptions(
        path: 'comandas/listar_clientes.php',
        queryParameters: {
          'empresa': (jsonDecode(escopo) as List)[1],
          'pesquisa': ''
        }));
